...
enum param_type
{
	illegal_param, disk_total_num, data_disk_num, disk_block_size, disk_raid_type, space_list_num, 
 ...
 * @return: param_type
 */
int FileSystemLayer::parse_setting_line(struct ncfs_state *ncfs_data, char *line)
{
...
{
	unlink("/tmp/ncfs_cmd");
	mkfifo("/tmp/ncfs_cmd",0666);
	//codingLayer = new CodingLayer;
...
int FileSystemLayer::round_to_block_size(int size)
{
	...
	disk_block_size = NCFS_DATA->disk_block_size;
	...
	if (magic_no == MAGIC_NUMBER){
		...
		for (i=0; i < ncfs_data->disk_total_num; i++){
			...
			if (retstat > 0){
				...
				ncfs_data->free_offset[disk_id] = metadata.free_offset;
				ncfs_data->free_size[disk_id] = metadata.free_size;
			}
		}

		//get disk space list info (deleted spaces)
		offset_space = sizeof(struct raid_metadata)*(ncfs_data->disk_total_num + 1);
...
void FileSystemLayer::get_operation_mode(struct ncfs_state *ncfs_data)
{
	...
	if (failed_disk_num == 0){
		...
		if (failed_disk_num <= (ncfs_data->disk_total_num - ncfs_data->data_disk_num)){  
                        mode = 1;
                }
                else{
                        mode = 2;
        ...
        else if (disk_raid_type == 13){  //Reed-Solomon coding
	  
                if (failed_disk_num <= (disk_total_num-data_disk_num)){
                        mode = 1;
                }
                else{
                        mode = 2;
        ...
        //Add by zhuyunfeng on 2011-05-30 end.
...
void FileSystemLayer::set_device_status(int diskid, int status){
	...
	if(NCFS_DATA->disk_status[diskid] == status)
		return;
	fprintf(stderr,"Set Disk %d to %d\n",diskid,status);

	NCFS_DATA->disk_status[diskid] = status;

	get_operation_mode(NCFS_DATA);
}
...
void FileSystemLayer::process_command(void){
	...
	//0:traditional rebuild method; 1:Dimakis' rebuild method
	int rebuild_method;
...
//	printf("%d\n",strcmp(command,"set"));
	if(strcmp(command,"recover") == 0){
		...
		scanf("%s %d %d",command,&failed_disk_id,&rebuild_method);
		printf("-%s -%d -%d\n",command,failed_disk_id,rebuild_method);
		//RecoveryTool::Recover(command,failed_disk_id,rebuild_method);
...
void FileSystemLayer::update_settting(void){
	...
	for(int i = 0; i < NCFS_DATA->disk_total_num; ++i){
		...
		long long totallen = (NCFS_DATA->free_offset[i] + NCFS_DATA->free_size[i]) * NCFS_DATA->disk_block_size;
...
void FileSystemLayer::readSystemConfig(struct ncfs_state* ncfs_data){
	...
	ncfs_data->w_cacuhy_rs = configLayer->getConfigInt("FileSystem>Disk>WValue");
	ncfs_data->disk_block_size = configLayer->getConfigLong("FileSystem>Disk>BlockSize");
	...
	//Add by zhuyunfeng on July 5,2011 begin.
	...
	for(int i = 0; i < ncfs_data->disk_total_num; ++i){
		...
		//Add by zhuyunfeng on July 4,2011 begin.
		//fetch IPAddress information for each storage node
		memset(tempString,0,256);
		sprintf(tempString,"%sIPaddr",targetDisk);
		
		const char* ipAddrTemp=configLayer->getConfigString(tempString);
		ncfs_data->ip_addr[i] = (char*)calloc(PATH_MAX,1);	
		strncpy(ncfs_data->ip_addr[i],ipAddrTemp,PATH_MAX);
		//Add by zhuyunfeng on July 4,2011 end.
