...
NetworkLayer::NetworkLayer(void){
	...
	pthread_create(&_ThreadID,NULL,&NetworkLayer::ServerThread,new thread_arg(this));
}
	...
	if(item.target == ALL){
		for(int i = 0; i <= _Maxfd; ++i){
			...
			if(ClientRecord[i].Connected)
				__retstat = sendn(i,item.payload,item.totallen);
			else
				continue;
			if(__retstat < 0)
				return 0;
		...
		if(ClientRecord[item.target].Connected)
			__retstat = sendn(item.target,item.payload,item.totallen);
		if(__retstat < 0)
			return 0;
		else
			++__count;
...
void NetworkLayer::AcceptNewClient(){
	...
	int __clientfd = accept(_ServerSocket,(struct sockaddr*)&__cliaddress,&__clilen);
...
void* NetworkLayer::ServerThread(void* arg){
	...
	while(1){
		...
		int readsocks = select(This->_Maxfd + 1, &socks, NULL,NULL,NULL);
		if(readsocks < 0){
			perror("select");
			exit(EXIT_FAILURE);
		} else if (readsocks == 0){
