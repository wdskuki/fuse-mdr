	...
	for(int i = _ServerSocket + 1; i <= _Maxfd; ++i){
		if(FD_ISSET(i,socks)){
			...
			if(retstat < 0){
				...
				fprintf(stderr,"Client disconnected %s %d\n",inet_ntoa(ClientRecord[i].Address.sin_addr),ntohs(ClientRecord[i].Address.sin_port));
				if(i == _Maxfd){
					do{
						--_Maxfd;
					}while((_Maxfd > 2) && !ClientRecord[_Maxfd].Connected);
				}
...
void ReportLayer::SendPath(int sd, char* header, int headersize, const char* path){
...
void ReportLayer::ModuleDispatcher(char Module, int id){
	switch(Module){
		case DISKUSAGE: diskusageLayer->ProcessRequest(id);break; 
		case FOLDER: FolderReader::ProcessRequest(id);break;
		default: break;
	}
}
