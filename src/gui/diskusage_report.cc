...
DiskusageReport::DiskusageReport(unsigned int numofblock) : TimedReport(1000){
	...
	for(int i = 0; i < NCFS_DATA->disk_total_num; ++i){
		...
		touched[i] = true;
		int __blocksize = NCFS_DATA->disk_block_size;
		unsigned int __usedblock = (NCFS_DATA->free_offset[i] - 1) / __blocksize;
	...
	_scale = (NCFS_DATA->free_offset[0] + NCFS_DATA->free_size[0]) / numofblock;
	start();
	//gettimeofday(&prev,NULL);
}

void DiskusageReport::Update(int diskid, int blockno, char status){
	if((diskid < 0) || (diskid > NCFS_DATA->disk_total_num) || (blockno < 0))
		return;
...
void DiskusageReport::SendOneDisk(int target, int diskid){
	...
	int freesize_n = htonl(NCFS_DATA->free_size[diskid] * NCFS_DATA->disk_block_size);
	memcpy(&content[6],&freesize_n,4);
	int freeoffset_n = htonl(NCFS_DATA->free_offset[diskid] * NCFS_DATA->disk_block_size);
