#ifndef __CODING_RDP_HH__
#define __CODING_RDP_HH__
...
class Coding4Rdp : public CodingLayer {
private:
	int k; //data disk num
	int **matrix; //row*col
	int row, col;
	...
	vector<int> rdp_find_q_blocks_id(int disk_id, int block_no);
...
public:
	...
	int rdp_recover_block(int disk_id, char *buf, long long size,
								long long offset, char*** pread_stripes, bool& in_buf);
	int rdp_recover_block2(int disk_id, char *buf, long long size,
		long long offset, char*** pread_stripes, bool& in_buf, double& func_cost_time);
	int rdp_get_strip_size(){return strip_size;}
