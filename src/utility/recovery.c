	...
	int src_disk_id;
		...
		for (j=0; j < ncfs_data->disk_total_num; j++){
			...
			if (src_disk_id != fail_disk_id[0]){
				fd_src = open(ncfs_data->dev_name[src_disk_id], O_RDONLY);
                		retstat = pread(fd_src, temp_buf, block_size, offset);
	...
	gf_gen_tables(field_power);
		...
		if (fail_disk_id[0] != code_disk_id){
					...
					(src_disk_id != code_disk_id)){
                        ...
                        for (j=0; j < ncfs_data->disk_total_num; j++){
				src_disk_id = j;
                                if ((j != parity_disk_id) && (j != code_disk_id)){
					fd_src = open(ncfs_data->dev_name[src_disk_id], O_RDONLY);
                                        //Cache Start
                                	retstat = pread(fd_src, temp_buf, block_size, offset);
                                        //Cache End
                                        ...
                                        data_disk_coeff = ncfs_data->disk_total_num - 3 - data_disk_coeff;
                                        data_disk_coeff = gf_get_coefficient(data_disk_coeff, field_power);

                                        for (k=0; k < block_size; k++){
                                                //calculate code block Q
                                                buf[k] = buf[k] ^
                                                (char)gf_mul((unsigned char)temp_buf[k],data_disk_coeff,field_power);
                                        }
					close(fd_src);
                                }
                        }
                                ...
                                int* fail_disk_id, char *target_dev)
{
	...
	(int)(2*((k*(n-1) - (int)(k*(k-1)/2)) + ((int)(n*(n-1)/2) - (int)(k*(2*n-k-1)/2)))/n);
...
int main(int argc, char *argv[])
{
	...
	//test print
    	printf("***main: disk_total_num=%d, data_disk_num=%d, block_size=%d, raid_type=%d\n",
	...
	if (fail_disk_num > 0){
		if (fail_disk_num == 1){
			if (ncfs_data->disk_raid_type == 1){
				...
				recover_mirror(ncfs_data, fail_disk_num, fail_disk_id,
									target_dev);
			}
			else if ((ncfs_data->disk_raid_type == 4)
					|| (ncfs_data->disk_raid_type == 5)){
				printf("recover_xor starts.\n");
				recover_xor(ncfs_data, fail_disk_num, fail_disk_id,
									target_dev);
			}
                        else if (ncfs_data->disk_raid_type == 6){
                                printf("recover_raid6 starts.\n");
                                recover_raid6(ncfs_data, fail_disk_num, fail_disk_id,
                                                                        target_dev);
                        }
			else if (ncfs_data->disk_raid_type == 1000){
				printf("recover_mbr starts.\n");
				recover_mbr(ncfs_data, fail_disk_num, fail_disk_id,
									target_dev);
			}
			else{
				printf("Recovery of this raid type is not supported\n");
			}
		}
		else{
